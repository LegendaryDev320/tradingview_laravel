        ...
        <div class="tab-content">
            <div class="tab-pane fade show active d-flex d-xl-block flex-wrap gap-2">
                ...
                <div class="trading-left__bottom sell-list-wrapper flex-fill">
                    <h5 class="trading-left__bottom-title d-block d-none d-xl-block order-book-price-all"  id="provider_price" style="color: #d1d4dc'">
                        <span class="market-price-{{@$pair->marketData->id}}" id="order-book-price-tag">
...
@push('script')
                            ...
                            if(resp.sell_side_orders.length <=0){
                                $('.sell-side-order-list').html(sellSideHtml || `
                                    <div class="empty-thumb">
                                        ...
                                        <p   class="empty-sell" style="color:##d1d4dc">@lang('No order found')</p>
                                    </div>
                                `);
                            }else{
    ...
    // Generate HTML for the order, ensuring rate and amount are displayed correctly
    return `
        <ul style="background: linear-gradient(to left, ${colorCode} ${ tradePercentage}%, transparent ${tradePercentage}%)" class="trading-left__list flex-between order-list skeleton mb-2 ps-0 order-rate-${order.rate.replace('.','_')} mb-2 ${order.has_my_order ? 'has-my-order' : ''}" data-rate="${order.rate}" data-total-amount='${order.total_amount}' data-total="${order.total}" >
            <li class="price__item">${showAmount(order.rate)}</li> <!-- Display rate in Open Price column -->
            <li class="amount__item">${showAmount(order.amount)}</li> <!-- Display amount in Amount column -->
            <li class="total__item">${showAmount(order.total)}</li> <!-- Display total in Total column -->
        </ul>
    `;
};
...
function orderPlace(data) {
    ...
    if (parseInt(data.order.order_side) === parseInt("{{ Status::SELL_SIDE_ORDER }}")) {
        parentElement = '.sell-side-order-list';
        $('.empty-sell').remove();
    } else {
        parentElement = '.buy-side-order-list';
