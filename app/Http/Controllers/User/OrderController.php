...
class OrderController extends Controller
{
    ...
    public function save(Request $request, $symbol)
    {
        ...
        if ($request->order_side ==  Status::BUY_SIDE_ORDER) {
            $details       = "Open order for buy coin on " . $pair->symbol;
            $walletBalance = $this->createTrx($wallet, 'order_buy', $amount, $charge, $details, $user);
        } else {
            $details       = "Open order for sell coin on " . $pair->symbol;
    ...
    public function bulkClose() {
        $orders = Order::with('pair')->where('user_id', auth()->id())
        ...
        $marketData = json_decode($marketDataJson, true);
    
        foreach ($orders as $order) {
            $jsonData = $marketData[$order->pair->type];
            $currentPrice = $jsonData[$order->pair->symbol];
            $lotValue = $order->pair->percent_charge_for_buy;
    
            $lotEquivalent = floatval($lotValue) * floatval($order->no_of_lot);
            $total_price = intval($order->order_side) === 2
                ? (floatval($order->rate) - floatval($currentPrice)) * $lotEquivalent
                : (floatval($currentPrice) - floatval($order->rate)) * $lotEquivalent;
    
            $this->close(request(), $order->id, $order->order_side, $total_price, $currentPrice, $total_price);
        }
    }
